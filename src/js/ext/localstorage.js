...
var lsCommandPluginFactory = function(md, emailProcessorBackend) {
  var commandsPlugin = function(mdkey, mdname, viewModel) {
    ...
    downloadPdfCmd.execute = function() {
      downloadPdfCmd.enabled(false);
      viewModel.notifier.info(viewModel.t("Downloading..."));
      viewModel.exportHTMLtoTextarea('#downloadPdfTextarea');
      var postUrl = emailProcessorBackend ? emailProcessorBackend : '/dl/';
      global.document.getElementById('downloadPdfForm').setAttribute("action", postUrl);
      global.document.getElementById('downloadPdfForm').submit();
      downloadPdfCmd.enabled(true);
    };

    viewModel.save = saveCmd;
    viewModel.test = testCmd;
    viewModel.download = downloadCmd;
    viewModel.downloadPdf = downloadPdfCmd;
