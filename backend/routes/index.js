...
router.post('/upload_editted/:id', function(req, res){
  var source = path.join(__dirname, "/../../dist/public/.tmp/", ""); // path where the editted image will be temporarily saved
  var imgName = path.join(path.parse(path.basename(url.parse(req.body.imgUrl).pathname)).name.replace('%20', '').replace(' ', '') + "_edit"); // the original name of image
  var destination = path.join(__dirname, "/../../dist/public/uploads/", req.params.id); // path where the editted image will be finally saved
  ...
  base64Img
    ...
    .then(function(result) {
      ...
      var url = "https://toping.io/uploads/" + req.params.id + "/" + imgName + extension;
      var thumbnailurl = "https://toping.io/uploads/" + req.params.id + "/thumbnail/" + imgName + extension;
      res.status(200).json({
        "url": url,
